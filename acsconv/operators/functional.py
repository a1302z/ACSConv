...
def acs_conv_f(
    ...
    **kwargs,
    ...
    C_out = weight.shape[1 if transposed else 0]
    assert (
        groups == 1 or groups == C_in == C_out
    ), "only support standard or depthwise conv"
    ...
    if transposed:
        ...
        weight_c = weight[
            :, acs_kernel_split[0] : (acs_kernel_split[0] + acs_kernel_split[1])
        ].unsqueeze(3)
    ...
    if acs_kernel_split[0] > 0:
        a = conv_func(
            x_a
            if conv3D_output_shape[0] == input_shape[0]
            or 2 * conv3D_output_shape[0] == input_shape[0]
            else F.pad(x, (0, 0, 0, 0, padding[0], padding[0]), "constant", 0)[
                :,
                :,
                kernel_size[0] // 2 : kernel_size[0] // 2
                + (conv3D_output_shape[0] - 1) * stride[0]
                + 1,
                :,
                :,
            ],
    ...
    if acs_kernel_split[1] > 0:
        c = conv_func(
            x_c
            if conv3D_output_shape[1] == input_shape[1]
            or 2 * conv3D_output_shape[1] == input_shape[1]
            else F.pad(x, (0, 0, padding[1], padding[1]), "constant", 0)[
                :,
                :,
                :,
                kernel_size[1] // 2 : kernel_size[1] // 2
                + stride[1] * (conv3D_output_shape[1] - 1)
                + 1,
                :,
            ],
            weight=weight_c,
    ...
    if acs_kernel_split[2] > 0:
        s = conv_func(
            x_s
            if conv3D_output_shape[2] == input_shape[2]
            or 2 * conv3D_output_shape[2] == input_shape[2]
                ...
                + stride[2] * (conv3D_output_shape[2] - 1)
                + 1,
            ],
            weight=weight_s,
            bias=None,
